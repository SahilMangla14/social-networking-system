...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RETURN QUERY VALUES (NULL, 'User does not exist.');
        RETURN;
    END IF;

    -- Loop through the array of request IDs
    FOR request_id IN SELECT unnest(p_request_ids) AS id
    LOOP
        ...
        IF NOT FOUND THEN
            RETURN QUERY VALUES (request_id, 'Follow request does not exist or is not pending for the user.');
            CONTINUE;
        END IF;
        ...
        RETURN QUERY VALUES (request_id, 'Follow request accepted.');
    END LOOP;
END;
$$
LANGUAGE plpgsql;
...
BEGIN
    ...
    RETURN QUERY SELECT new_like_id;
...
BEGIN
    ...
    RETURN QUERY SELECT new_message_id;
...
BEGIN
    ...
    FOR member_id IN SELECT unnest(p_member_ids) AS id
    LOOP
...
BEGIN
    ...
    LEFT JOIN (
        ...
        GROUP BY
            post_id
    ) pcomment ON p.id = pcomment.post_id
    WHERE
        p.user_id = p_user_id
    ORDER BY
        p.created_at DESC;
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM follow_request
        WHERE source_user_id = p_viewer_user_id
